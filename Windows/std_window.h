...
*/
enum class WindowOptions : std::int32_t
{
...
private:
	...
	std::vector<std::filesystem::path> m_Fonts;			// Font List (Full Paths)
	std::vector<std::string> m_FontStems;				// Font List (Filename Stems)
	float m_FontSize;									// Current Font Size
	float m_FontSizeMin;								// Minimum Font Size
	float m_FontSizeMax;								// Maximum Font Size
...
public:
	...
	explicit Standard_Window(void) :
		...
		b_Maximized(false)
	{
		static std::once_flag StdWinIsWindows10OrGreaterFlag;

		std::call_once(StdWinIsWindows10OrGreaterFlag, [this]() {
		...
		static std::once_flag StdWinInitCommonControlsExFlag;

		std::call_once(StdWinInitCommonControlsExFlag, [this]() {
		...
		static std::once_flag StdWinFontListCreatedFlag;

		std::call_once(StdWinFontListCreatedFlag, [this]() {
			m_Fonts = GetFonts();
			m_FontStems.reserve(m_Fonts.size());
			for (const auto& Font : m_Fonts) { m_FontStems.push_back(Font.filename().stem().string()); }
			});
	}
	...
	Standard_Window(Standard_Window&& v) noexcept :
		...
		m_FontStems(std::exchange(v.m_FontStems, std::vector<std::string>())),
		m_FontSize(std::exchange(v.m_FontSize, 0.0f)),
		m_FontSizeMin(std::exchange(v.m_FontSizeMin, 0.0f)),
		m_FontSizeMax(std::exchange(v.m_FontSizeMax, 0.0f)),
	...
	*/
	[[nodiscard]] operator const bool&() const { return hWnd; }

	/*
		Is the window active?
	*/
	[[nodiscard]] const bool& IsActive(void) const { return b_IsActive; }

	/*
		Does the window have focus?
	*/
	[[nodiscard]] const bool& HasFocus(void) const { return b_HasFocus; }

	/*
		Is the window fullscreen?
	*/
	[[nodiscard]] const bool IsFullscreen(void) const;
	...
	*/
	[[nodiscard]] const bool IsMinimized(void);

	/*
		Is the window maximized?
	*/
	[[nodiscard]] const bool IsMaximized(void);

#if MSTD_DEVICE
	/*
		Windows Devices
	*/
	[[nodiscard]] const std::shared_ptr<Windows_Devices>& Device(void) const { return m_Devices; }
#endif
	...
	*/
	...
	/*
		Is toolbar enabled?
	*/
	[[nodiscard]] const bool& IsToolBar(void) const { return b_Toolbar; }

	/*
		Toolbar handle
	*/
	[[nodiscard]] const HWND& GetToolBar(void) const { return h_ToolBar; }
	...
	*/
	[[nodiscard]] const bool& IsStatusBar(void) const { return b_StatusBar; }

	/*
		Status bar handle
	*/
	[[nodiscard]] const HWND& GetStatusBar(void) const { return h_StatusBar; }

	/*
		Status bar parts
	*/
	[[nodiscard]] const INT& GetStatusBarParts(void) const { return m_StatusBarParts; }
	...
	*/
	...
	/*
		Get device context
	*/
	[[nodiscard]] const HDC& GetDeviceContext(void) const { return GetDC(hWnd); }

	/*
		Get window handle
	*/
	[[nodiscard]] const HWND& Get(void) const { return hWnd; }

	/*
		Get parent window handle
	*/
	[[nodiscard]] const HWND& GetParent(void) const { return hWndParent; }

	/*
		Get instance handle
	*/
	[[nodiscard]] virtual const HINSTANCE& GetInstance(void) const override { return h_Instance; }

	/*
		Get window style
	*/
	[[nodiscard]] const DWORD& GetStyle(void) { return m_Style = GetWindowLong(hWnd, GWL_STYLE); }

	/*
		Get old window style
		- used for restore after fullscreen
	*/
	[[nodiscard]] const DWORD& GetOldStyle(void) const { return m_OldStyle; }

	/*
		Get old window client rect
		- used for restore after fullscreen
	*/
	[[nodiscard]] const RECT& GetOldRect(void) const { return m_OldRect; }

	/*
		Get window extended style
	*/
	[[nodiscard]] const DWORD& GetStyleEx(void) { return m_StyleEx = GetWindowLong(hWnd, GWL_EXSTYLE); }
	...
	*/
	[[nodiscard]] const StringW& GetCaptionName(void) const { return s_Name; }

	/*
		Get class name
	*/
	[[nodiscard]] const StringW& GetClassname(void) const { return s_ClassName; }
	...
	*/
	[[nodiscard]] COLORREF& GetColor(void) { return m_Color; }

	/*
		Get current font path
		 - full path to font file
	*/
	[[nodiscard]] const std::filesystem::path& GetFont(void);
	...
	*/
	[[nodiscard]] std::size_t& FontIndex(void) { return m_CurrentFontIndex; }

	/*
		Get a full path list of Truetype Fonts
	*/
	[[nodiscard]] const std::vector<std::filesystem::path>& FontList(void) const { return m_Fonts; }

	/*
		Get filename stem list of Truetype Fonts
	*/
	[[nodiscard]] const std::vector<std::string>& FontStems(void) const { return m_FontStems; }
	...
	*/
	...
	/*
		Get font minimum size
	*/
	[[nodiscard]] float& FontSizeMin(void) { return m_FontSizeMin; }

	/*
		Get font maximum size
	*/
	[[nodiscard]] float& FontSizeMax(void) { return m_FontSizeMax; }

	/*
		Get brush
	*/
	[[nodiscard]] HBRUSH& GetBrush(void) { return h_Brush; }

	/*
		Get class brush
	*/
	[[nodiscard]] HBRUSH& GetClassBrush(void) { return m_Class.hbrBackground; }
	...
	*/
	...
	/*
		Get menu handle
	*/
	[[nodiscard]] const HMENU& GetMenuHandle(void) { return h_Menu = GetMenu(hWnd); }
	...
	*/
	[[nodiscard]] const HACCEL& GetAcceleratorTable(void) const { return h_AccelTable; }

	/*
		Get DPI
	*/
	[[nodiscard]] const UINT& GetDPI(void) { return m_WindowDPI = GetDpiForWindow(hWnd); }

	/*
		Get system DPI
	*/
	[[nodiscard]] const UINT& GetSystemDPI(void) { return m_SysDPI = GetDpiForSystem(); }

	/*
		Get window DPI scale factor
	*/
	[[nodiscard]] const FLOAT& GetDpiScaleFactor(void) { return static_cast<FLOAT>(m_WindowDPI = GetDpiForWindow(hWnd)) / USER_DEFAULT_SCREEN_DPI; }
	...
	*/
	[[nodiscard]] const INT& GetDeviceScaleFactorPercentage(void) const { return std::to_underlying(m_ScaleFactor); }
	...
	*/
	[[nodiscard]] const BOOL& GetDisallowPeek(void) const { return b_DisallowPeek; }
	...
	*/
	[[nodiscard]] const BOOL& GetExcludeFromPeek(void) const { return b_ExcludeFromPeek; }
	...
	*/
	...
	/*
		Get dark mode
	*/
	[[nodiscard]] const BOOL& GetDarkMode(void) const { return b_DarkMode; }
	...
	*/
	[[nodiscard]] const DWM_WINDOW_CORNER_PREFERENCE& GetRoundCorners(void) const { return m_RoundCorners; }
	...
	*/
	...
	/*
		Get border color
	*/
	[[nodiscard]] COLORREF& GetBorderColor(void) { return m_BorderColor; }
	...
	*/
	[[nodiscard]] COLORREF& GetBorderColorOnLostFocus(void) { return m_BorderColorLostFocus; }
	...
	*/
	...
	/*
		Get caption color
	*/
	[[nodiscard]] COLORREF& GetCaptionColor(void) { return m_CaptionColor; }
	...
	*/
	[[nodiscard]] COLORREF& GetCaptionColorOnLostFocus(void) { return m_CaptionColorLostFocus; }
	...
	*/
	...
	/*
		Get text color
	*/
	[[nodiscard]] COLORREF& GetTextColor(void) { return m_TextColor; }
	...
	*/
	[[nodiscard]] COLORREF& GetTextColorOnLostFocus(void) { return m_TextColorLostFocus; }
	...
	*/
	...
	/*
		Get backdrop type
	*/
	[[nodiscard]] const DWM_SYSTEMBACKDROP_TYPE& GetBackdropType(void) const { return m_BackdropType; }

	/*
		Get dropped files
	*/
	[[nodiscard]] StrVecW& GetDroppedFiles(void) { return s_DroppedFiles; }

	/*
		Get child window count
	*/
	[[nodiscard]] const std::size_t& GetChildWindowCount(void) const { return v_ChildWindows.size(); }

	/*
		Get child window
	*/
	[[nodiscard]] const std::shared_ptr<Standard_Window>& GetChildWindow(UINT Index);
	...
	*/
	void MsgDisplayChange(WPARAM wParam, LPARAM lParam) const;
