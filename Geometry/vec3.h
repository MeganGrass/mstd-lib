...
public:
	...
	*/
	...
	/*
		Construction
	*/
	explicit vec3(void) noexcept : x(0.0f), y(0.0f), z(0.0f) {}
	explicit vec3(float X, float Y, float Z) noexcept : x(X), y(Y), z(Z) {}
	explicit vec3(std::int32_t X, std::int32_t Y, std::int32_t Z) noexcept : x(static_cast<float>(X)), y(static_cast<float>(Y)), z(static_cast<float>(Z)) {}
